   ...
   if(empty($errors)){
        ...
        $result = $db->query($query);
         if($result && $db->affected_rows() === 1){
          ...
          //failed
          $session->msg('d',' Désolé de ne pas avoir mis à jour le groupe!');
          redirect('edit_group.php?id='.(int)$e_group['id'], false);
        }
   } else {
...
<!--     *************************     -->
        <div class="form-group">
              <label for="name" class="control-label">Modifier le nom du groupe</label>
        ...
        <div class="form-group">
          <label for="status">Status</label>
              <select class="form-control" name="status">
                ...
                <option <?php if($e_group['group_status'] === '0') echo 'selected="selected"';?> value="0">Deactivé</option>
              </select>
        </div>
<!--     *************************     -->
