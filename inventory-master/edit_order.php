...
<?php
if(isset($_POST['modifier la commande'])){
  $customer = remove_junk($db->escape($_POST['client']));
  ...
  if(empty($errors)){
     ...
     if($result && $db->affected_rows() === 1) {
       $session->msg("s", "Commande mise à jour avec succès");
       redirect('orders.php',false);
     } else {
       $session->msg("d", "Pardon! Échec de la commande");
       redirect('orders.php',false);
     }
  } else {
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
         <strong>
           ...
           <span>Modification de l'ordre #<?php echo remove_junk(ucfirst($order['id']));?></span>
        </strong>
       </div>
         ...
         <form method="post" action="edit_order.php?id=<?php echo (int)$order['id'];?>">
           ...
           <div class="form-group">
                    <select class="form-control" name="paymethod">
                     ...
                     <option value="Cash" <?php if($order['paymethod'] === "Cash" ): echo "selected"; endif; ?> >En espèces</option>
                     <option value="Check" <?php if($order['paymethod'] === "Check" ): echo "selected"; endif; ?> >Chéque</option>
                     <option value="Credit" <?php if($order['paymethod'] === "Credit" ): echo "selected"; endif; ?> >Credit</option>
                     <option value="Charge" <?php if($order['paymethod'] === "Charge" ): echo "selected"; endif; ?> >Charge au compte</option>
                    </select>
           ...
           <button type="submit" name="edit_order" class="btn btn-primary">Mise à jour de la commande</button>
       </form>
       </div>
     </div>
